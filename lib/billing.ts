 ...
 * - Past that, time is billed in half-hour steps with a 15-minute grace:
 *   leave 21:00–21:15 → 21:00, leave 21:16–21:30 → 21:30, etc.
 */
...
/** Billable hours between start and checkout (min 2h, half-hour steps, 15-min grace). */
export function billedHours(start: Date, end: Date): number {
  const minutes = Math.max(0, (end.getTime() - start.getTime()) / 60_000);
  const steps = Math.max(0, Math.ceil((minutes - 15) / 30));
